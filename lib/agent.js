...
// Central model and summary refresh policy used across the agent flow.
...
async function extractCrossChatMemories(groq, conversation, answer, summary) {
  const normalizedConversation = normalizeMessages(conversation);
  ...
  for (let step = 0; step < 5; step += 1) {
    ...
    if (!message.tool_calls || message.tool_calls.length === 0) {
      ...
      if (isSingleMessage && sessionId && chatId) {
        ...
        const candidateMemories = await extractCrossChatMemories(
          groq,
          updatedConversation,
          answer,
          chat.summary
