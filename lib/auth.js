...
/*
 * PHASE 1 LEARNING GUIDE — IDENTITY / GOOGLE AUTH
 *
 * Phase 1 supports two owner identities:
 *   Guest:  browser-generated local session id
 *   Signed in: "google:<Google subject id>"
 *
 * Google OAuth proves identity, then Talli stores a small signed HttpOnly
 * cookie containing the user's Google identity fields. The signature prevents
 * the browser from silently changing the cookie payload without detection.
 *
 * There is currently no automatic guest-to-Google data migration. Signing in
 * changes the owner namespace used by chats and memories, so this is an
 * important behavior to remember while rebuilding the frontend in Phase 1B.
 */
import { createHmac, randomUUID, timingSafeEqual } from "node:crypto";

// Google OAuth and signed-cookie session helpers. The rest of the app only
// needs to know whether a request belongs to a Google user or a guest session.
...
const AUTH_RATE_LIMITS = {
  me: { limit: 60, windowMs: 60 * 1000 },
  "google-start": { limit: 10, windowMs: 10 * 60 * 1000 },
  "google-callback": { limit: 20, windowMs: 10 * 60 * 1000 },
  logout: { limit: 20, windowMs: 60 * 1000 },
};
...
// A signed token is trusted only when its HMAC signature matches. timingSafeEqual
// avoids a normal string comparison for this security-sensitive check.
...
// Minimal cookie parsing to avoid bringing in an extra middleware layer.
...
function getAuthAction(req) {
  return getQueryParam(req, "action") ?? "me";
}
...
function consumeRateLimit(req, action) {
...
function applyRateLimit(req, res, action) {
  const { limited, retryAfterSeconds } = consumeRateLimit(req, action);
  ...
  if (req.method === "GET" && action !== "me") {
    return res.redirect(`/?authError=rate_limited`);
  }
...
// Google OAuth configuration is read from environment variables so the same
// code can run locally, on Vercel production, or on preview deployments.
...
function getMissingGoogleAuthEnvVars() {
  const missing = [];
...
// The rest of the app persists data by owner id. Authenticated Google users
// take precedence; otherwise we fall back to the browser session id.
...
// Exported so both the local Express server and the serverless route wrapper
// can apply auth rate limiting before the request reaches the auth handler.
export function enforceAuthRateLimit(req, res) {
  return applyRateLimit(req, res, getAuthAction(req));
}
...
// Single auth endpoint that exposes config inspection, Google sign-in start,
// callback handling, and logout.
  ...
  try {
    ...
    if (req.method === "GET" && action === "me") {
      return res.status(200).json({
        ...
        missingGoogleAuthEnvVars: getMissingGoogleAuthEnvVars(),
      });
    }
    ...
    // OAuth callback security: the random state value must match the signed
    // state cookie created before redirecting to Google. This helps prevent
    // cross-site request forgery in the sign-in flow.
