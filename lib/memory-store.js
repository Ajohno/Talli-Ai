...
/*
 * PHASE 1 LEARNING GUIDE — CROSS-CHAT MEMORY
 *
 * Chat history and long-term memory are separate systems. This module stores
 * small durable facts that can be reused across multiple conversation threads.
 *
 * Current Phase 1 memory lifecycle:
 *   agent extracts candidate facts
 *        -> normalize category/content/confidence
 *        -> hash content to deduplicate
 *        -> upsert into MongoDB
 *        -> rank memories for a future request
 *        -> inject the best matches into the model context
 *
 * Retrieval is intentionally lightweight right now: keyword overlap,
 * confidence, and a small recency signal. It is not embedding/vector search.
 */
...
// The SHA-256 key is based on normalized text content. If essentially the same
// exact fact is extracted again, upsertMemories updates the existing record
// instead of creating an endless duplicate list.
...
// Only a small ranked subset is sent to the model. This avoids dumping every
// stored memory into every prompt and keeps context more relevant/manageable.
