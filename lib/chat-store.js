...
// Chat persistence lives in MongoDB. This module is intentionally focused on
// shaping, validating, trimming, and storing chat data only.
...
// Keep each chat bounded so prompts do not grow without limit.
...
// Convert raw MongoDB documents into the API shape consumed by the UI.
...
// One-time compatibility bridge for older session records.
...
// Persist the trimmed conversation while preserving a stable title once the
// user has meaningfully named the chat with their first message.
...
// Summaries are stored separately from the raw conversation so the agent can
// keep thread continuity even after older messages are trimmed away.
