...
/*
 * PHASE 1 LEARNING GUIDE — CHAT PERSISTENCE
 *
 * Think of one MongoDB chat document as one conversation thread:
 *
 *   owner/sessionId
 *   chatId
 *   title
 ...
 *   timestamps
 *
 * sessionId is the owner namespace (guest browser id OR google:<sub>).
 * chatId identifies one thread inside that owner's namespace.
 *
 * Two kinds of context are intentionally different:
 *   1. conversation + summary = context for THIS thread
 *   2. memories collection    = reusable context ACROSS threads
 *
 * The raw conversation is capped, while the summary preserves older thread
 * context. This prevents prompts from growing forever.
 */
...
// Phase 1 keeps at most MAX_CHAT_MESSAGES raw messages. Older details can
// still survive through the separate summary generated by lib/agent.js.
...
// Creating a chat writes a real MongoDB document immediately; it is not only
// a temporary frontend object. The current default thread starts with Talli's
// greeting message, which the new UI can choose how to present visually.
...
// Archiving is deliberately reversible: it only flips archived and updates
// updatedAt. The chat history and summary stay intact for restoration.
